...
int main() {
	...
	*/
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::rotate( model, 0.5f, glm::vec3( -3.0f, 0.0f, 0.0f ) ); // use to compare orthographic and perspective projection
		model = glm::scale(model, glm::vec3(4.0f, 2.0f, 1.0f));
		//view = glm::translate( view, glm::vec3( screenWidth / 2, screenHeight / 2,-50.0f ) ); // use with orthographic projection
		...
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model)); // <-- FALTABA
		glDrawArrays(GL_TRIANGLES, 0, 36);

		// ---- Cubo 3 (izquierda) ----
		glm::mat4 model3 = glm::mat4(1.0f);
		model3 = glm::translate(model3, glm::vec3(-5.0f, 0.0f, 0.0f)); 
		model3 = glm::scale(model3, glm::vec3(2.5f, 3.0f, 1.0f));
